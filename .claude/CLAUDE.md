...
The image is a Python 3.14 slim base with `uv sync --frozen --no-dev`; it carries no ffmpeg and no compiler, because nothing in the app needs them. A push to `main` builds and publishes `ghcr.io/trighele/ffsum:latest`; Watchtower, already running on the maintainer's Unraid box, notices the new digest and restarts the container — there is no orchestrator, no deploy step in CI, and nothing in the pipeline that touches the box directly. The container itself is defined once, in Unraid's Docker template UI, which this repo cannot see or version: it maps host port `4301` to the container's `8000`, host path `/mnt/user/appdata/ffsum` to `/data`, and supplies `ANTHROPIC_API_KEY`, `CLAUDE_MODEL`, and `FFSUM_DATA_DIR` as template variables. A manual `workflow_dispatch` run also publishes, but under a dated `dev-<YYYYMMDD>-<sha>` tag that Watchtower does not watch, so a branch build never reaches the live container.
